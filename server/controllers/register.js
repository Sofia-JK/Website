...
export const register = async (req, res) => {
    ...
    const {nickname, adress, fio, namber, password} = req.body;
    
    //кандидат это переменная в которую попытаемся найти и записать пользователя с таким никнеймом
    const candidate = await sql`select * from Users where nickname = ${nickname} limit 1`[0]
    ...
    await sql`insert into Users(nickname, adress, fio, namber, role, password) values(${nickname}, ${adress}, ${fio}, ${namber}, ${userRole[0].role}, ${hashPassword})`
